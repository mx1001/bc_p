...
from ... graphic.draw_px import draw_extending_circle
...
class BoxCutterDrawCircle(bpy.types.Operator):
        """Draw a box from 2d shape """
        bl_idname = "boxcutter.draw_circle"
        bl_label = "Box Cutter Draw Circle"
        ...
        def execute(self, context):
            ...
            generate_3d_mesh(self, context, 'CIRCLE')
        ...
        def modal(self, context, event):
            ...
            elif event.type == 'LEFTMOUSE' and event.value == 'RELEASE' and not event.shift and not event.alt :
                check_if_moving(self, context, event)

                set_evet_region(self, event, False)
                append_vert_to_list(self, context, event.mouse_region_x, event.mouse_region_y)
                bpy.types.SpaceView3D.draw_handler_remove(self._handle, 'WINDOW')
            ...
            elif event.type == 'LEFTMOUSE' and event.value == 'RELEASE' and not event.shift and event.alt :
                check_if_moving(self, context, event)
            ...
            if event.type == "WHEELUPMOUSE" or event.type == 'NUMPAD_PLUS' and event.value=='PRESS':
                if self.n <= 127 :
                    self.n += 1
                    bpy.context.scene.BoxCutter_circle_vert_count = self.n
                else:
                    pass
        ...
        def invoke(self, context, event):
            bpy.context.scene.BoxCutter_help_item = 'CIRCLE'

            if bpy.context.scene.BoxCutter_shape_moving == True:
                self.savedx = saved_cords[0]
                self.savedy = saved_cords[1]

            self.yellow_color = False
            self.n = bpy.context.scene.BoxCutter_circle_vert_count
            self.use_3d_cursor = True
            if context.area.type == 'VIEW_3D':
                self.first_active = bpy.context.scene.objects.active
                boxcutter_show_wire(self, context, True)
                args = (self, context)
                self._handle = bpy.types.SpaceView3D.draw_handler_add(draw_extending_circle, args, 'WINDOW', 'POST_PIXEL')
